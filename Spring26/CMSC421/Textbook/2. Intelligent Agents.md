...
- Actions are selected based on the CURRENT percept, **ignoring history** (basing actions on a single camera frame, etc).
- Condition-action rules: simple reflex relation formulated by "if \_\_\_\_\_ then \_\_\_\_\_" 
![[Pasted image 20260204220557.png]]
- Simplicity is what we want to target with AI agents, but simple reflex agents have limited intelligence.
	- Big issues with even a little bit of unobservability.
	- ex: simple reflex of "if car ahead braking, then brake" can be disastrous if the car ahead does not have an easily identifiable brake light.
- Can fall victim to infinite loops for simple reflex agents in a partially observable environment (forever attempting and failing the same action).
	- Can escape if agents input randomization on actions. But random actions are not rational in single-agent environments, thus nonoptimal.
### 2.4.3 Model-based reflex agents
- Effective method of worlds with partial visibility. Have agents keep track of the part of the world it cannot see, i.e. keep an internal state of the world based on percept history.
- With the car example, our agent can now determine when a car is braking based off of the change in brightness of their taillights.
- This internal state requires two kinds of information:
	- Transition model: Information about the world's changes over time, "how the world works".
	- Sensor model: How the changes in the world appears to the agent's sensors.
		- Knowledge of what to expect when cameras get wet, obscuring vision.
- Agents that use these models to keep track of the world with best guesses are called model-based agents.
![[Pasted image 20260204221917.png]]
### 2.4.4 Goal-based agents
- Knowing the current state of the environment is not always enough.
	- ex: at junctions, do you turn right, left, or drive forward?
- Goal information is used to describe situations that are desirable, such as being on the correct road/path/route.
- The agent program can combine this with the model (model-based agents) to choose actions that achieve the goal.
![[Pasted image 20260204224529.png]]
- Bottom line, goal-based agents will have to consider the future and consequences of their actions - whether they aid in pursuing the goal.
	- ex: goal-based agents will brake when they see brake lights not because that's what they're programmed to do, rather they brake because crashing will result in not achieving their goal of successful driving.
### 2.4.5 Utility-based agents
- Goals alone aren't enough to generate high-quality behavior in most environments.
- Utility-based agents don't just achieve any solution, they aim to achieve the best solution to meet a goal.
	- This model allows the agent to perform comparisons of different world states, figuring out which one is "happier". Utility is another way of saying "happy" or "unhappy", utility is a term that just sounds better.
- Agent has an internal performance measure called a utility function that internally regulates the agent.
	- Distinction: Performance measures are what the researchers use to grade the agent. Utility functions allow the agent to judge themselves.
- Advantages in flexibility and learning.
	- For example, two goals that contradict each other. The utility function can help decide best tradeoffs to maximize the expected utility of action outcomes.
![[Pasted image 20260207154339.png]]
- Choosing the utility-maximizing course of action is an incredibly difficult task.
- Perfect rationally is usually unachievable in practice due to computation complexity.
- Additionally, not all utility agents are model based, there are model-free agents as well.
### 2.4.6 Learning agents
- So far we have described agent programs with purposes of selecting actions. Now we dive into agent programs that "come into being".
- The learning agent approach is the preferred method for many areas of AI.
- Any type of agent (model, utility, goal) can be built as a learning agent.
- Learning allows the agent to operate in initially unknown environments, becoming more competent overtime.
- Learning agents can be split into 4 elements:
	- Critic
		- Provides feedback to the learning element with respect to a fixed performance standard. Basically gives context to the percept that is fed into later components. This component can be thought of as "outside the agent".
	- Learning element
		- Responsible for making improvements.
		- Provides feedback to the performance element, of how it could do better in the future.
	- Problem generator
		- Suggests actions that will lead to new and informative experiences.
		- The curiosity to guide the agent to take risks with different approaches in hopes of learning better methods.
	- Performance element
		- Responsible for selecting external actions.
		- Takes in percepts and outputs actions (basically the entirety of the previous agents we have learned), but is now supported by 3 other components.
![[Pasted image 20260207163634.png]]
### 2.4.7 How the components of agent programs work
- We can place representations of the environment that the components process on a line of increasing complexity and expressive power.
	- Atomic $\rightarrow$ Factored $\rightarrow$ Structured.
	- Atomic: each state of the world if indivisible. No internal structure.
		- Simplest blank representation, where every entity is a black box. We only know whether one entity is identical or different to another entity.
	- Factored: splits each state of the world into a fixed set of variables/attributes, each of which can have a value.
		- Adds more detail by giving each entity the same set of attributes. This way entities can share similarities and differences.
	- Structured: Provides the relationship links between different entities. Describes the world in First-Order Logic.
		- Factored can be seen as a rigid structure, immutable, hardcoded. Whereas Structured can be seen as a dynamically resizing structure you can add/remove elements to/from.
		- Attributes aren't hardcoded with True/False, rather they are object representations such as `Color(Obj1, Blue)`.
- Consider the component that deals with "What my (the agent's) actions do".
	- Describes the changes that might occur in the environment as the result of its actions.
	- ![[Pasted image 20260207170301.png|500]]
- Chess as an example:
	- Atomic: requires around $10^{38}$ pages to represent.
	- Factored: requires thousands of pages written with propositional logic.
	- Structure: can be represented with a page or two of first-order logic.
- Another axis of representation involves the mapping of concepts to locations in physical memory.
	- One-to-one mappings between concepts and memory locations are called *localist representations* (atomic).
		- Pros: Easy to interpret and distinguishable from other concepts.
		- Cons: Once forgotten/damaged the concept is gone. Inefficient since you need to specifically allocate one unit for that concept. Lack of generalization.
	- If the concept representation is spanned across numerous memory locations then it is a *distributed representation* (factored/structured).
		- Pros: Generalizable and robust since the concept is represented with building blocks that are used in other concepts too. Efficient due to shared units.
		- Cons: Complex to interpret. Learning new concepts may interfere with old concepts if there is much conceptual overlap.