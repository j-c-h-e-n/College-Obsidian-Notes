...
- A* is great but it expands a lot of nodes, i.e. it consumes a lot of memory and is slow.
	- We can compromise with "good enough" solutions for the sake of speed, which are called satisficing solutions.
- Using an inadmissible heuristic (one that may overestimate the true distance to the goal), we can get a speed up with a slight risk of missing the optimal solution. But we will still at least obtain a good enough solution.
- Weighted A* provides an algorithm that places greater emphasis on the heuristic, where the estimated "to-go" cost is given more weight than the "already traveled" cost.
	- $f(n) = g(n) + W*h(n)$ for some $W>1$. (AKA Anytime A* with $\epsilon = W$)
- We can observe that by manipulating $W$, we can see representations of the logic of many other searches:
	- $W=1$: A*, $g(n) + 1*h(n)$.
	- $W=0$: Uniform cost search, $g(n)$.
	- $W=\inf$: Greedy best-first search, $h(n)$.
	- $1<W<\inf$: Weighted A*, $g(n) + W*h(n)$.